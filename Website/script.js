...
function translateImage(image) {
    console.log("translateImage", image);
...
function annotateImage(translations) {
    ...
    for (let i = 0; i < translations.length; i++) {
        ...
        let textToSpeechElem = document.createElement("audio");
        textToSpeechElem.controls = "controls"

        // File path, format and media type
        let mp3FileElem = document.createElement("source");
        mp3FileElem.src = translations[i]["audio_info"]["fileUrl"];
        mp3FileElem.type = "audio/mpeg"; //MP3

        textToSpeechElem.appendChild(mp3FileElem);
        translationsElem.appendChild(document.createElement("hr"));
        translationsElem.appendChild(translationElem);
        translationsElem.appendChild(textToSpeechElem);
    }
}
